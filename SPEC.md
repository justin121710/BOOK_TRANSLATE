...
| 直排 | **完整還原**，含右→左欄序 |
| 圖片 | 原樣裁切貼回；**圖內日文直接取代**：蓋掉原字再把中譯寫上去 |
...
| 原本 | 改為 | 原因 |
|---|---|---|
| 直排標點 `。、` 置於字格右上角 | **正常置中** | 使用者看過實際效果後的決定。角落是傳統慣例但視覺偏舊。`rules.js` 的 `PUNCT_MODE` 可切回 |
| 圖內日文以註解形式列在圖旁／頁尾 | **直接取代**：蓋掉原字再寫中譯 | 使用者決定。填色取自文字框四周的中位色而非純白，才不會在彩色插圖上留下白斑 |
| 翻譯僅用 Claude | **Gemini 與 Claude 可切換**，預設 Gemini | 有免費額度。注意：先前以為可與 Vision 共用金鑰是錯的，Generative Language API 不支援 referrer 限制 |
...
- **warp.js** — 使用者拖四角 → 3×3 homography → 重投影
- **enhance.js** — 縮放、去陰影（縮圖重度模糊估背景 → 除法正規化）、編碼
- 去陰影後的影像**只給 OCR 用**；貼回 PDF 的插圖一律用未處理的彩色影像

**透視校正的實作路線（實測後改過一次）**：

一開始用 canvas2d 的網格法（把畫面切成三角形逐塊仿射貼圖）。它很快（3ms），
但**會在紙面上留下規律的對角線接縫**：三角形之間必須外推一點才不會有空隙，
而外推讓反鋸齒邊緣被合成兩次，淺色紙面上就顯出格線。實測空白紙面的相鄰像素
跳變率 0.02%，肉眼在整頁上看得出網格。

改成 **WebGL 片段著色器逐像素反向 homography** 後跳變率降到 **0.00%**，
耗時 20–30ms，數學上精確且完全無接縫。網格法保留為 `forceMesh` 備援，
在沒有 WebGL 的環境仍可運作。

實測（`_scratch/warptest.html`）：合成書頁 → 套梯形變形 → 校正回來，
四角標記全部歸位，中央取樣為紙色。
