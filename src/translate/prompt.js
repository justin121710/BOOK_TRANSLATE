...
輸入的日文來自光學辨識（OCR），**必然會有讀錯的字**。常見的情況：

- 字形相近而誤認（例：己↔已↔巳、辻↔迂、木↔本、ロ↔口、ー↔一↔│）
- 假名的濁點半濁點掉了（例：ば→は、ぱ→は）或多出來
- 小字的促音拗音被讀成大字（例：っ→つ、ゃ→や）
- 標點誤認（例：。→o、、→ヽ）或整個漏掉
- 兩個字被黏成一個、或一個字被拆成兩個

遇到明顯不通順、不成詞、或違反文法的地方，**請依前後文推斷原本應該是什麼字，
按照修正後的意思翻譯**，不要照著錯字硬翻，也不要因為看不懂就略過。

只有在你有把握是辨識錯誤時才修正。如果那個寫法本來就可能成立
（罕見詞、人名、方言、作者的刻意用字），維持原樣。

每當你做了這種修正，請在 correctedSource 欄位回報修正後的完整日文原文，
好讓使用者知道你依據什麼翻譯。沒有修正就不要填這個欄位。

## 數學公式的佔位符

原文裡可能出現 ⟦M1⟧、⟦M2⟧ 這種記號。那是從原書裁下來的**數學公式圖片**，
不是文字。請遵守：

- **原樣保留**，一個都不能少、不能多、不能改寫、不能翻譯。
- 把它擺在中文句子裡**語意上該在的位置**。中文的語序和日文不同，
  位置通常會跟著移動，這是正確的，不要為了對齊原文而硬留在原處。
- 佔位符代表一個數學物件（變數、符號、算式），翻譯時請把它當成一個名詞看待。
- 不要在佔位符之間插入解釋，也不要把它展開成文字。

例：原文「したがって⟦M1⟧は⟦M2⟧に比例する。」→「因此 ⟦M1⟧ 與 ⟦M2⟧ 成正比。」
        ...
        items: {
          ...
          properties: {
            ...
            correctedSource: {
              type: 'string',
              description:
                '只有在你判斷 OCR 讀錯字、並依前後文修正後才填：修正過的完整日文原文。' +
                '沒有修正就不要填這個欄位。',
            },
          },
